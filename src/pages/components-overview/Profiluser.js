...
const [uploadedImage,setUploadedImage]=useState("")
  ...
  const handleFileInputChange  = (e) => {
    if (e.target.files[0]) {
      ...
      console.log("image consultant**************",imageFile)
    }
    console.log("***************",image)
    ...
    const handleImageUpload = async() => {
      ...
      if (image) {
          ...
          formData.append('nameFolder', 'consultantImages');
              ...
              const response = await axios.post(`${process.env.REACT_APP_URL}/file/`, formData, { headers: { Authorization:` Bearer ${token}` } })
              console.log("image id ***",response.data)
              setImageid(response.data)
             const imid=response.data

              const resp = await  axios.post(`${process.env.REACT_APP_URL}/consultants/saveImage/${user.id}/${imid}`,null,
              {
                headers: {
        ...
        <div className="container py-5">
          ...
          />
                  ...
                  <div className="d-flex justify-content-center mb-2">
                     <button type="button" className="btn btn-outline-primary ms-1" onClick={handleEditProfileClick}>   
                      {showConsultant ? "Annuler la modification" : "Modifier ton profil"}
                      </button> 
